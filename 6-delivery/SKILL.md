...
6. **CONTRACT_ID ИЗ ИМЕНИ ПАПКИ = ЕДИНСТВЕННЫЙ ПЕРВИЧНЫЙ КЛЮЧ.** Суффикс после последнего `_` в имени папки (напр. `Elena_02021988_77f5f456` → `77f5f456`) — это первые 8 символов contract_id. Искать строку в Sheet ТОЛЬКО по `col F starts_with(этот суффикс)`. Затем ПРОВЕРИТЬ: col H (ДР) == ДР из CSV. Любое расхождение → СТОП. Подробный алгоритм — правило N в `2-money-analysis/money_quality_gate.md`.
