...
# ── Z. Запрет ЦЕПОЧЕК диспозиторов (только узел → его прямой диспозитор) ─
# Дарья (Дарья/Ольга/Анна/Валентина, 26–29.06.2026): «только до диспозитора
# идём, никаких цепочек». Легально: «диспозитор Раху/Кету — <Планета>»
# (именительный падеж — называем прямого диспозитора узла). Цепочка = взятие
# диспозитора У НЕ-узловой планеты: «диспозитор Венеры…», «диспозитор
# диспозитора…» (родительный падеж планеты) = FAIL.
CHAIN_DISPOSITOR = re.compile(
    r"диспозитор\w*\s+(?:диспозитор\w*|венеры|марса|меркури[яю]|юпитера|"
    r"сатурна|солнца|луны|плутона|урана|нептуна)",
    flags=re.IGNORECASE,
)


def check_dispositor_chains(text):
    hits = sorted(set(m.group(0) for m in CHAIN_DISPOSITOR.finditer(text)))
    if hits:
        add("FAIL", f"Z. Цепочки диспозиторов ({len(hits)} шт.): {', '.join(hits[:6])} — оставить только один шаг: узел → его прямой диспозитор Раху/Кету. «Диспозитор диспозитора» ЗАПРЕЩЁН.")
    else:
        add("PASS", "Z. Цепочек диспозиторов нет (только узел → его прямой диспозитор).")


# ── AA. Ровно 1 интеграционная сфера в блоке «11 сфер» ───────────────
# Дарья (Анна/Валентина): «интеграционная сфера только 1, всегда».
def check_integration_count(text):
    block = get_spheres_block(text)
    if not block:
        return
    m = re.search(r"интеграцион", block, flags=re.IGNORECASE)
    if not m:
        add("FAIL", "AA. Не найдена интеграционная сфера (нужна ровно 1 — синтез всех 10).")
        return
    tail = block[m.start():]
    integ_spheres = re.findall(r"^\s*\d+\.\s+\*\*", tail, flags=re.MULTILINE)
    if len(integ_spheres) == 1:
        add("PASS", "AA. Ровно 1 интеграционная сфера.")
    else:
        add("FAIL", f"AA. Интеграционных сфер {len(integ_spheres)} — должна быть РОВНО 1 (всегда). Свести к одной.")
...
def main():
    ...
    check_dispositor_chains(text)            # Z  — запрет цепочек диспозиторов
    check_integration_count(text)            # AA — ровно 1 интеграционная сфера
