...
## Конфигурация (`токены/` внутри репо скиллов)
...
Cloudflare deploy-токены, которые меняют код воркеров, в `токены/` не кладём. Pull/delivery должны запускаться из содержимого скиллов без обращения к `intergalactic_workers_ai/config`.
